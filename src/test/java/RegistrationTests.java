...
public class RegistrationTests {
    ...
    static void beforeAll() {
        ...
        void successfulRegistrationTest {
            ...
            $("#gender-radio-1").parent().click(); // ищем радиокнопку через родителя и тапаем на нее
//        $("#genterWrapper").$(byText("Female")).click(); не очень хороший пример с выбором кнопки но для определ локали
//        $("label[for="gender-radio-3]").click(); ищем радиокнопку через лэйбл и тапаем на нее
            $("#userNumber").setValue("79995552211");
            $("#dateOfBirthInput").click(); // выбор даты рождения в календаре
            $(".react-datepicker__month-select").selectOption("September");
//        $(".react-datepicker__month-select").selectOptionByValue("6");
            $(".react-datepicker__year-select").selectOption("1988");
            $(".react-datepicker__day--030:not(.react-datepicker__day--outside-month)").click(); // выбираем день в месяце
...
//        $("#react-select-3-option-0").click();
            ...
            $(".table-responsive").shouldHave(text(userName), text("Bokov"),
                    text("evgenii@mail.ru"), text("79995552211"));
        }
